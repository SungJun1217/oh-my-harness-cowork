...
**Provenance is structural, not decorative.** Don't wrap agent-derived slots (`PLAN?`, `FAIL`) in quotes/fences to mark them as data: the slot name already carries provenance (invariant 3), the header already says the block is notes, not instructions, and the research above found structural provenance effective while content-level markers weren't — extra fencing only spends the 900-byte budget. `NOTE` keeps its name (it comes from a deliberate `omhc note` command, usually typed by the human; `NOTE?` would mislabel human notes), but notes are stamped and expire after `due.MAX_AGE_SECONDS` so a stale note can't ride along in every handoff (#36).

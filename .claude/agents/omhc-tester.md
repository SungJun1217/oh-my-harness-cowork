...
- Known sandbox traps (measured 2026-09-24, Claude Code 2.1.281 / codex-cli 0.155.1):
  ...
  - Headless `claude -p` sessions carry `entrypoint:"sdk-cli"` and omhc deliberately never hands
    them off. To get a Claude→Codex source session, either have the user run one interactive
    session, or copy the sandbox JSONL and flip `entrypoint` to `"cli"` — and say you did.

...
- **신뢰되지 않은 Codex 훅은 여전히 Claude→Codex 방향을 통째로 끕니다.**
  ...
  설정 변경이라 고치지 않았습니다. **알려진 구멍(일부 검증됨):**
  `session_meta.timestamp`는
  롤아웃 첫 줄에서 한 번만 읽고 다시 갱신하지 않습니다. 실제 머신에서 측정:
  한 세션의 롤아웃 파일이 첫 레코드와 마지막 레코드 사이 며칠(한 사례에서
  80시간)의 활동을 담고 있으면서도 그 첫 줄 타임스탬프는 그대로였습니다 —
  그래서 오래 이어지거나 재개된 세션이 이미 그 세션을 대상으로 한 번 돈 백필
  보다 더 새것으로 보이지 않을 수 있습니다. 그리고 그 원래 시작 시각이
